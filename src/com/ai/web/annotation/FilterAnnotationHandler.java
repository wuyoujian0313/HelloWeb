...
@WebFilter(filterName = "FilterAnnotationHandler")
public class FilterAnnotationHandler implements Filter {
    private ServletContext servletContext = null;

    /* 过滤器初始化时扫描指定的包下面使用了WebServletEx注解的那些类
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        //如果配置了多个包，例如：<param-value>com.ai.web</param-value>
    ...
    private void addServletClassToServletContext(String packageName,Map<String, Class<?>> classMap){
        Set<Class<?>> setClasses =  ClassUtil.getClasses(packageName);
        for (Class<?> clazz :setClasses) {
            if (clazz.isAnnotationPresent(WebServletEx.class)) {
                //获取WebServletEx这个Annotation的实例
                WebServletEx annotationInstance = clazz.getAnnotation(WebServletEx.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (uri.indexOf("!")==-1) {
            ...
            WebServletEx annotationInstance = clazz.getAnnotation(WebServletEx.class);
            //获取注解上配置的初始化参数数组
            WebInitParamEx[] initParamArr = annotationInstance.initParams();
            Map<String, String> initParamMap = new HashMap<String, String>();
            for (WebInitParamEx initParam : initParamArr) {
